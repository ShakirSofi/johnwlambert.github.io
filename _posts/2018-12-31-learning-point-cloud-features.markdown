...
single network to predict point grouping proposals
...
in an embedded feature space

$$
L = L_{sim} + L_{cf} + L_{sem}
$$

The matrix $$S \in \mathbf{R}^{N_p \times N_p}$$, and element $$S_{ij}$$ classifies whether or
not points $$P_i$$ and $$P_j$$ belong to the same object instance.
Each row of $$S$$ can be viewed as a proposed grouping of
points that form a candidate object instance.

We obtain $$S$$ by, for each pair of
points $$\{Pi, Pj\}$$, simply subtracting their corresponding feature vectors $$\{F_{sim_i}, F_{sim_j} \}$$ and taking the $$\ell_2$$ norm such that $$S_{ij} = \|F_{sim_i} − F_{sim_j} \|_2 $$
...
$$
L_{sim} = \sum\limits_{i}^{N_p} \sum\limits_{j}^{N_p}l(i, j)
$$
...
\max(0, K_2 − \|F_{sim_i} − F_{sim_j} \|_2) & C_{ij} = 3 \end{cases}
$$
...
many of which are noisy or represent the same object.

discard proposals with predicted confidence less than
...
T hM1 is set to 0.6 and T hM2 is set to 200. T hC is
set to 0.1. Our network is implemented
...
naive method like Seg-Cluster tends to
